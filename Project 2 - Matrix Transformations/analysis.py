...
class Analysis:
    ...
    def min(self, headers, rows=[]):
        ...
        '''
        # Empty arr to store the min vals from each header's column
        minCols = np.empty(len(headers))
        # Getting the select data we're working with
        arr = self.data.select_data(headers, rows)
        # Making an arr of the min vals in each col
        minCols = arr.min(axis=0)
        # Returning the np array with the min vals for each col
        return minCols
    ...
    def max(self, headers, rows=[]):
        ...
        NOTE: There should be no loops in this method!
        '''
        # Empty arr to store the max vals from each header's column
        maxCols = np.empty(len(headers))
        # Getting the select data we're working with
        arr = self.data.select_data(headers, rows)
        # Making an arr of the max vals in each col
        maxCols = arr.max(axis=0)
        # Returning the np array with the max vals for each col
        return maxCols
    ...
    def range(self, headers, rows=[]):
        ...
        NOTE: There should be no loops in this method!
        '''
        # Returning arrs with the mins and the maxes
    ...
    def mean(self, headers, rows=[]):
        ...
        arr = self.data.select_data(headers, rows)
        mean = np.empty(len(headers))
        mean = np.sum(arr, axis=0)
    ...
    def var(self, headers, rows=[]):
        ...
        '''
        arr = self.data.select_data(headers, rows)
        var = np.empty(len(headers),)
        var = arr - self.mean(headers, rows)
        var = var ** 2
        var = np.sum(var, axis=0)
        var = var/(arr.shape[0]-1)
        return var
    ...
    def std(self, headers, rows=[]):
        ...
        NOTE: There should be no loops in this method!
        ''' 
        std = np.sqrt(self.var(headers, rows))
        return std
    ...
    def pair_plot(self, data_vars, fig_sz=(12, 12), title=''):
        ...
        # begin by creating subplots
        figure, axes = plt.subplots(len(data_vars), len(data_vars), figsize = fig_sz, sharex=False, sharey=False)
        
        # storing the indices we need for the variables passed in
        indList = []
        ...
        # plot the points in the subplots
        for i in range(len(data_vars)):
            x = self.data.data[:, indList[i]]
            for j in range(0, len(data_vars)):
