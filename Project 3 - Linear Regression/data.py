...
class Data:
    ...
    def __str__(self):
        ...
        '''
        # Adding filepath, samples, dimensions, and header names
        dataStr = str(self.filepath) + ' (' + str(self.data.shape[0]) + 'x' + str(self.data.shape[1]) + ')' + '\nHeaders:\n' + '   '.join(self.headers)

        # Showing first 5 rows of data only and mentioning total num samples
        if self.get_num_samples() < 5:
            rangeVal = self.get_num_samples()
        else: 
            rangeVal = 5
        dataStr = dataStr + '\n-------------------------------\n' + 'Showing first ' + str(rangeVal) + '/' + str(self.data.shape[0]) + ' rows.'

        # add all < 5 or top 5 rows to string and return the final value
        for i in range(rangeVal):
            
            rowAsString = ''
            for floatVal in self.get_sample(i):
                rowAsString = rowAsString + str(floatVal)+ '   '
            dataStr = dataStr + '\n' + rowAsString
    ...
    def select_data(self, headers, rows=[]):
        ...
        for header in headers:
            cols.append(headerList[header.strip()])
        if rows != []:
