...
class PCA_COV:
    ...
    def covariance_matrix(self, data):
        ...
        '''
        # creating a column of means 
        col_means = data.mean(axis=0)
        # creating Ac
        Ac = data - col_means
        # creating the covar matrix
        covar = (Ac.T @ Ac) / (Ac.shape[0] - 1)
    ...
    def compute_prop_var(self, e_vals):
        ...
        '''
        # list to hold the proportion variance
        prop_var = []
        for e_val in e_vals:
            #prop var is list of eigenvalues/total_of_eigenvalues
            prop_var.append((e_val)/sum(e_vals))
        return prop_var
    ...
    def compute_cum_var(self, prop_var):
        ...
        '''
        # A list to hold the cumulative variances
        cum_var = []
        total_var = 0
    ...
    def pca(self, vars, normalize=False):
        ...
        '''
        self.A = self.data.filter(items = vars)
        self.A = self.A.to_numpy()
        self.vars = vars
        orig_A = self.A
        self.orig_mins = orig_A.min(axis=0)
        self.orig_range = orig_A.max(axis = 0) - orig_A.min(axis=0)

        if normalize == True: 
            self.A = (orig_A - self.orig_mins) / self.orig_range
    ...
    def pca_project(self, pcs_to_keep):
        ...
        col_means = np.mean(self.A, axis = 0)
        self.A_proj = (self.A - col_means) @ pcs
        return self.A_proj
    ...
    def pca_then_project_back(self, top_k):
        ...
        col_means = self.A.mean(axis = 0)
        Ar = (self.A_proj @ vhat.T) + col_means

        if self.normalized == True:
            Ar = Ar * self.orig_range + self.orig_mins
        return Ar
