...
def count_words(email_path='data/enron'):
    ...
    folders = os.listdir(email_path)
    if len(folders) == 3:
        folders.remove('.DS_Store')
    
    for folder in folders:
        folder = os.path.join(email_path, folder)
        files = os.listdir(folder)
        totalEmails += len(files)
...
def find_top_words(word_freq, num_features=200):
    ...
    for i in range(len(top_words)):
        freq.append(word_freq.get(top_words[i]))
    ...
    for folder in folders:
        ...
        for filename in os.listdir(folder):
            ...
            with open(os.path.join(folder, filename), 'r') as filedata:
                ...
                words = tokenize_words(string)
                for i in range(len(top_words)):
                    count = words.count(top_words[i])  
                    feats[finalIdx, i] = count
            finalIdx += 1
...
def make_train_test_sets(features, y, test_prop=0.2, shuffle=True):
    ...
    # Your code here:
    trainIdx = features.shape[0]*(1-test_prop)
    trainIdx = int(trainIdx)
    x_train = features[:trainIdx, :]
    x_test = features[trainIdx:, :]

    ytrainIdx = len(y)*(1-test_prop)
    ytrainIdx = int(ytrainIdx)
    y_train = y[:ytrainIdx]
    y_test = y[ytrainIdx:]

    indstrainIdx = len(inds)*(1-test_prop)
    indstrainIdx = int(indstrainIdx)
    inds_train = inds[:indstrainIdx]
    inds_test = inds[indstrainIdx:]
