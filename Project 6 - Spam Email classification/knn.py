...
class KNN:
    ...
    def predict(self, data, k):
        ...
        copy = np.copy(data)
        numY = copy.shape[0]
        predictedY = np.zeros((numY,))

        for i in range(numY):
            dist = np.sqrt(np.sum(((self.exemplars - copy[i,:])**2), axis = 1))
            sorted_dist = dist.argsort()
            labels = self.classes[sorted_dist[:k]]
            count = np.bincount(labels.astype('int'))
            predictedY[i] = np.argmax(count)
    ...
    def plot_predictions(self, k, n_sample_pts):
        ...
        '''
        ls_arr = np.linspace(-40,40, n_sample_pts)

        x,y = np.meshgrid(ls_arr, ls_arr)
        xy = np.column_stack((x.flatten(), y.flatten()))
        predictedY = self.predict(xy, k).reshape((n_sample_pts, n_sample_pts))

        figure, axes = plt.subplots(1,1, figsize = (10,5))
        figure.colorbar(axes.pcolormesh(x, y, predictedY, cmap = ListedColormap(cartocolors.qualitative.Safe_4.mpl_colors)))
