...
class NaiveBayes:
    ...
    def train(self, data, y):
        ...
        for i in range(self.num_classes):
            for label in y:
                if label == i:
                    class_priors[i] += 1
        self.log_class_priors = np.log(class_priors / len(y))

        self.log_class_likelihoods = np.zeros((self.num_classes, num_features))
        for i in range(num_features):
            for j in range(num_samps): 
                self.log_class_likelihoods[int(y[j]), i] += data[j,i]
        for class_id in range(self.num_classes):
            self.log_class_likelihoods[class_id] = (self.log_class_likelihoods[class_id] + 1)/ ((np.sum(self.log_class_likelihoods[class_id])) + num_features)
            self.log_class_likelihoods[class_id] = np.log(self.log_class_likelihoods[class_id])
    ...
    def predict(self, data):
        ...
        predicted = np.zeros(num_test_samps)
        for i in range(num_test_samps):
            log_posterior = self.log_class_priors + self.log_class_likelihoods @ data[i].T
            predicted[i] = np.argmax(log_posterior, axis = 0)
